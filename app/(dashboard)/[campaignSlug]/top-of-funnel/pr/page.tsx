...
export default function PrMediaDetailPage() {
  ...
  const kpiMetrics: KpiMetric[] = [
    ...
    {
      label: "TONE",
      value: lastField(prRows, "tone"),
      format: "text",
    },
  ];

  // ── Chart data ────────────────────────────────────────────────────────────
  const mentionsMonthly = toMonthly(prRows, "mentions");
  const mentionsQuarterly = toQuarterly(prRows, "mentions", "sum");

  const earnedMonthly = toMonthly(prRows, "earned_pct");
  const earnedQuarterly = toQuarterly(prRows, "earned_pct", "avg");

  const sovMonthly = toMonthly(prRows, "sov_pct");
  const sovQuarterly = toQuarterly(prRows, "sov_pct", "avg");
    ...
    <GranularityProvider>
      <div className="min-h-screen bg-page">
        ...
        <PageHeader
          ...
          subLabel="Top of Funnel · PR / Media"
          dateRange={dateRange}
        ...
        <main className="max-w-[1160px] mx-auto px-6 py-10">
          ...
          <section className="flex flex-col gap-4">
            <ChartPair
              metric="Mentions"
              monthlyData={mentionsMonthly}
              quarterlyData={mentionsQuarterly}
              color={TOF_GREEN}
              fillColor={TOF_FILL}
            />
            <ChartPair
              metric="Earned (%)"
              monthlyData={earnedMonthly}
              quarterlyData={earnedQuarterly}
              color={TOF_GREEN}
              fillColor={TOF_FILL}
            />
            <ChartPair
              metric="SOV (%)"
              monthlyData={sovMonthly}
              quarterlyData={sovQuarterly}
              color={TOF_GREEN}
              fillColor={TOF_FILL}
            />
          </section>
