...
export default function DeepLinksDetailPage() {
  ...
  // ── Chart data ────────────────────────────────────────────────────────────
  const clicksMonthly = toMonthly(deeplinksRows, "clicks");
  const clicksQuarterly = toQuarterly(deeplinksRows, "clicks", "sum");

  const installsMonthly = toMonthly(deeplinksRows, "installs");
  const installsQuarterly = toQuarterly(deeplinksRows, "installs", "sum");

  const ctoMonthly = toMonthly(deeplinksRows, "cto_pct");
  const ctoQuarterly = toQuarterly(deeplinksRows, "cto_pct", "avg");
      ...
      <div className="min-h-screen bg-page">
        ...
        <main className="max-w-[1160px] mx-auto px-6 py-10">
          ...
          <section className="flex flex-col gap-4">
            ...
            <ChartPair
              metric="Installs"
              monthlyData={installsMonthly}
              quarterlyData={installsQuarterly}
              color={BOF_AMBER}
              fillColor={BOF_FILL}
            />
            <ChartPair
              metric="CTO (%)"
              monthlyData={ctoMonthly}
              quarterlyData={ctoQuarterly}
              color={BOF_AMBER}
              fillColor={BOF_FILL}
            />
          </section>
